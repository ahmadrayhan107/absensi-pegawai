...
<body class="bg-blue-100">
  ...
  <div class="top-0 left-0 z-10 h-fit w-full bg-blue-100 py-2">
    ...
    <form action="" method="post" class="container mb-3 flex items-center justify-center md:justify-end">
      ...
      <button type="submit" name="submit" id="submit"
        ...
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="h-[20px] w-full">
  ...
  <!-- Content Start -->
  <div class="container">
    <table class="mt-12 table w-full table-auto border-collapse border text-xs md:text-base">
      <tr>
        <th class="border-collapse border border-slate-400 px-1.5 py-1.5 text-secondary">
          No
        </th>
        <th class="border-collapse border border-slate-400 px-1.5 py-1.5 text-secondary">
          ID Admin
      ...
      foreach ($data['admin'] as $row) { ?>
        <tr>
          <td class="border-collapse border border-slate-400 px-1.5 py-1.5 text-secondary text-center">
            <?= $no; ?>
          </td>
          <td class="border-collapse border border-slate-400 px-1.5 py-1.5 text-secondary text-center">
            <?= $row['id_admin']; ?>
          ...
          <td class="w-auto border-collapse border border-slate-400 px-3 py-2 text-center text-secondary md:w-[154px]">
            <div class="flex w-full">
              <div class="w-5/12 md:w-1/2">
                <a href="<?= BASE_URL; ?>/Admin/edit/<?= $row['id_admin'] ?>">
                  <button type="button"
              ...
              <div class="ml-2 w-5/12 md:w-1/2">
                <a href="<?= BASE_URL; ?>/Admin/remove/<?= $row['id_admin'] ?>" onclick="return confirm('Hapus Data?');">
                  <button type="button"
                    class="w-full rounded-lg bg-primary py-1.5 text-center text-white shadow-sm hover:opacity-80">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"
                      class="h-[20px] w-full">
